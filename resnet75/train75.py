...
# =========================
# 1B. Dropout Wrapper (External, does NOT modify model file)
# =========================
...
class ResNetWithDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)  # drop features before FC
...
def build_resnet(num_classes=2):
    if hasattr(resnet_mod, "generate_model"):
        return resnet_mod.generate_model(
            model_depth=101,
            n_classes=num_classes,
            shortcut_type="B",
            n_input_channels=3,
        )
    raise RuntimeError("Cannot find resnet model")
...
class NiADVideoDataset(Dataset):
    def __init__(self, root_dir, phase="Training", frames_per_clip=30, augment=False):
...
def train_improved():
    ...
    base_model = build_resnet(num_classes=2)
    model = ResNetWithDropout(base_model, dropout_p=0.4).to(DEVICE)
    ...
    # Load datasets
    train_dataset = NiADVideoDataset(ROOT_DIR, "Training", 30, augment=True)
    val_dataset = NiADVideoDataset(ROOT_DIR, "Validation", 30, augment=False)
    ...
    # Train loader (Option A: no sampler)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            save_checkpoint(model, optimizer, epoch, "best_resnet.pth")
        else:
            patience_counter += 1
